...
void pedirNotas(float arregloNotas[], char nombre[]);
...
int main(void) {
	...
	int opcion = 0;
	float promedio = 0.0;
	//1. EL EXPEDIENTE (Regla N + 1)
	char nombreEstudiante[30];


	printf("=== SISTEMA DE AUDITORÍA NIVEL 8===\n");
	printf("Ingrese el nombre completo del estudiante:  ");

	//El escudo perfecto antiscanf"
	fgets(nombreEstudiante, 30, stdin);
	nombreEstudiante[strcspn(nombreEstudiante, "\n")] = '\0';

	//2. Delegamos el ingreso de notas llamando a la función pedirNotas
	pedirNotas(calificaciones, nombreEstudiante);

	//3. Delegamos los cálculos matemáticos llamando a las respectivas funciones.
	...
	do {
		...
		if (scanf("%d", &opcion) !=1) { 
			...
			switch(opcion) {
				case 1:
				printf("\n[RESULTADO] El promedio de las notas de %s es: %.2f\n",nombreEstudiante,  promedio);
				break;

				case 2:
				printf("\n[RESULTADO] La nota más alta de %s es: %.2f\n",nombreEstudiante, max);
				break;

				case 3:
				printf("\n[RESULTADO] La nota más baja de %s es: %.2f\n",nombreEstudiante, min);
				break;
...
void pedirNotas(float arregloNotas[], char nombre[]) {
	...
	for (i = 0; i < 5; i++) {
		do {
			printf("Ingrese la nota del examen %d de %s: ", i+1, nombre);
