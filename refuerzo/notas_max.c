...
int main (void) {
    ...
    for (i = 0; i < 5; i++) {
        do {
            ...
            if (calificaciones[i] > 10 || calificaciones[i] < 0) {
                printf("[!] ERROR Rango invalido, intente de nuevo.\n");
            }
        // AQUI ESTABA EL ERROR 1: El while debe ir pegado a la llave de cierre del do
        } while (calificaciones[i] > 10 || calificaciones[i] < 0); 
    }
    ...
    } while (opcion != 4); /*El bucle se repite siempre que la opción no sea 4*/

    return 0;
} // AQUI ESTABA EL ERROR 2: Faltaba esta llave para cerrar el main
