...
dinámica ocupada*/
int guardar_radar(const listaRastreo *lista, const char *ruta_archivo);
int cargar_radar(listaRastreo *lista, const char *ruta_archivo);
...
int main(void) {
	...
	//FASE 0: Inyección de memoria a largo plazo.
	printf("\n---Fase  0: Restaurando historial---\n");
	cargar_radar(&mi_radar, "red_logs.txt");
	...
	//Fase 4.5: Extracción al Disco(Persistencia)
	printf("\n---Fase 4.5: Volcando memoria RAM al disco duro---\n");
	//Utilizamos un archivo local llamado "red_logs.txt"
	if (!guardar_radar(&mi_radar, "red_logs.txt")) {
			fprintf(stderr, "[X] ALERTA: La sesión no pudo ser guardada.\n");
	}
...
//Vuelca el estado de la memoria RAM hacia el disco duro.
int guardar_radar(const listaRastreo *lista, const char *ruta_archivo) {
	//Escudo antibasura: Validamos que haya algo qué guardar y una ruta válida.
	if (lista == NULL || lista->cabeza == NULL || ruta_archivo == NULL) {
		fprintf(stderr, "[X] Abortando guardado, radar vacío o ruta nula.\n");
		return 0;
	}
	//1. Apertura del canal: Solicitamos acceso al S.O en modo "w" (escritura).
	FILE *archivo = fopen(ruta_archivo, "w");

	//Escudo: Validamos si Linux nos concedió el acceso al archivo.
	if (archivo == NULL) {
			perror("[X] Fallo crítico al crear o abrir el archivo físico");
			return 0;
	}

	Nodo *actual = lista->cabeza; //Instanciamos el explorador.
	int nodos_guardados =  0;

	//2. Ciclo  de volcado: Recorremos el tejido conectivo.
	while (actual  != NULL) {
			if (actual->registro_ip != NULL) {
					/*Inyectamos la cadena en el archivo físico seguida de un salto de 
					línea*/
					fprintf(archivo, "%s\n", actual->registro_ip);
					nodos_guardados++;

			}
			actual =  actual->siguiente; //Avanzamos al siguiente bloque.
		}
		//3. Blindaje final: Cerramos el canal para evitar fugas de descriptores de archivos
		fclose(archivo);

		printf("[+] Extracción exitosa: %d IPs guardadas en el '%s'.\n", nodos_guardados, ruta_archivo);
		return 1;

	}
//Inyecta el  historial del disco duro hacia la memoria RAM.
int cargar_radar(listaRastreo *lista, const char *ruta_archivo) {
		//Escudo antibasura: Validamos los  punteros de entrada.
		if (lista == NULL || ruta_archivo == NULL) return 0;


		//1. Apertura del canal: Solicitamos acceso  en modo lectura  ("r")
		FILE *archivo =  fopen(ruta_archivo, "r");

		//Escudo de S.O: Si el archivo no existe.
		if (archivo == NULL) {
				printf("[i] INFO: Archivo de historial '%s' no encontrado. Iniciando radar  en blanco.\n", ruta_archivo);
				return 0;
		}

		//2. Preparación de la sonda: Buffer temporal para guardar el texto leído.
		char buffer_ip[50];
		int  nodos_cargados =0;

		//3. Ciclo de extracción: fgets lee  línea por línea  hasta el final del archivo.
		while  (fgets(buffer_ip, sizeof(buffer_ip),  archivo) != NULL) {
			//Sanitización: fgets captura  el "Enter" y luego lo extirpamos.
			buffer_ip[strcspn(buffer_ip, "\r\n")] =  0;
			if (strlen(buffer_ip) > 0) {
					insertar_final(lista, buffer_ip);
					nodos_cargados++;
			}
		}

		//4. Blindaje final: cerramos el archivo.
		fclose(archivo);

		printf("[+]Inyección exitosa: %d IPs restauradas desde '%s'.\n", nodos_cargados, ruta_archivo);
		return 1;
}
